...
const featuredProjects = [
  ...
  },
  {
    ...
    description:
      "End-to-end detection and response platform, installed from a clone with one command: Java/Spring Boot ingestion gateway (syslog UDP + HTTP, 12 log parsers), Python/FastAPI correlation engine, React dashboard, plus two endpoint agents — a Java/JNA EDR sensor (process lineage, file-integrity monitoring, on-demand YARA, persistence snapshots, remote response) and a host telemetry agent (metrics, network flows, users, eBPF on Linux). Detection is deterministic end to end: 18 static rules, 14 community Sigma rules run through an in-house compiler, 5 correlation rules, 400 named attack-tool and LOLBin signatures, IP reputation and statistical anomaly detection, with every alert mapped to MITRE ATT&CK. Aegis Total adds static triage of uploaded samples: PE/ELF/Mach-O, Office and PDF, entropy, suspicious imports, YARA signatures, extracted IOCs and entry-point disassembly with the API calls resolved on the imports. SIEM side: OCSF-aligned event schema with deduplication and sequence-gap detection, search across 24 filterable fields, configurable retention and OCSF export. SOAR playbooks with dry-run preview, per-action risk/reversibility metadata and rollback for the reversible ones; response actions include process-tree kill, quarantine and persistence removal. Security engineering: mTLS enrollment on an internal PKI, RBAC, full audit log. 653 backend tests plus 134 JUnit tests on the Java sensor, end-to-end API smoke; the container stack runs in ~630 MB and the two host agents add ~430 MB, with AI off.",
